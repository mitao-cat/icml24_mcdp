...
if __name__ == "__main__":
    ...
    for step, (X, y, s) in enumerate(train_infinite_loader):
        ...
        if step % args.log_freq == 0 or step == 1 or step == args.num_training_steps:
            ...
            train_metrics = test(model=net, test_loader=train_loader, criterion=criterion, device=device,  prefix="train", args=args)
            val_metrics   = test(model=net, test_loader=val_loader, criterion=criterion, device=device, prefix="val", args=args)
            test_metrics  =  test(model=net, test_loader=test_loader, criterion=criterion, device=device, prefix="test", args=args)
            res_dict = {}
