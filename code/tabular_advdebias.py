...
from networks import AdvDebiasing
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="advdebias", help="Model type")
    ...
    for step, (X, y, s) in enumerate(train_infinite_loader):
        ...
        advfairnet = train_step(model=advfairnet, data=X, target=y, sensitive=s, optimizer=optimizer, scheduler=scheduler,  clf_criterion=clf_criterion, adv_criterion=adv_criterion, lam=args.lam,  device=device)

        if step % args.log_freq == 0 or step == 1 or step == args.num_training_steps:

            train_metrics = test(model=advfairnet, test_loader=train_loader, clf_criterion=clf_criterion, adv_criterion=adv_criterion, lam=args.lam, device=device, prefix="train", args=args)
            val_metrics   = test(model=advfairnet, test_loader=val_loader,   clf_criterion=clf_criterion, adv_criterion=adv_criterion, lam=args.lam, device=device, prefix="val", args=args)
            test_metrics  = test(model=advfairnet, test_loader=test_loader,  clf_criterion=clf_criterion, adv_criterion=adv_criterion, lam=args.lam, device=device, prefix="test", args=args)
            res_dict = {}
