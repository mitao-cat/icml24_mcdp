...
def test(model, test_loader, clf_criterion, fair_criterion, lam, beta, device, target_index = 31, sensitive_index = 20, prefix="test", args=None):
    with torch.no_grad():
        model.eval()
        target_hat_list = []
        target_list = []
...
if __name__ == '__main__':
    ...
    for step, (X, attr) in enumerate( train_infinite_loader ):
        ...
        net = train_step(model=net, data=X, target=y, sensitive=s, optimizer=optimizer, scheduler=scheduler,  clf_criterion=criterion, fair_criterion=fair_criterion, lam=args.lam, device=device, beta=args.beta)

        if step % args.log_freq == 0 or step == 1 or step == args.num_training_steps:

            val_metrics   = test(model=net, test_loader=val_loader, clf_criterion=criterion, fair_criterion=fair_criterion, lam=args.lam, beta=args.beta, target_index = target_index, sensitive_index = sensitive_index, device=device, prefix="val", args=args)
            test_metrics  =  test(model=net, test_loader=test_loader, clf_criterion=criterion, fair_criterion=fair_criterion, lam=args.lam, beta=args.beta, target_index = target_index, sensitive_index = sensitive_index, device=device, prefix="test", args=args)
            res_dict = {}
