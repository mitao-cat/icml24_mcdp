...
def test(model, test_loader, criterion, device, target_index = 31, sensitive_index = 20, prefix="test", args=None):
    with torch.no_grad():
        model.eval()
        target_hat_list = []
        target_list = []
...
if __name__ == '__main__':
    ...
    for step, (X, attr) in enumerate( train_infinite_loader ):
        ...
        net = train_step(model=net, data=X, target=y, sensitive=s, optimizer=optimizer, scheduler=scheduler,  criterion=criterion, device=device)

        if step % args.log_freq == 0 or step == 1 or step == args.num_training_steps:

            val_metrics   = test(model=net, test_loader=val_loader, criterion=criterion, target_index = target_index, sensitive_index = sensitive_index, device=device, prefix="val", args=args)
            test_metrics  =  test(model=net, test_loader=test_loader, criterion=criterion, target_index = target_index, sensitive_index = sensitive_index, device=device, prefix="test", args=args)
            res_dict = {}
    ...
    with torch.no_grad():   # Final eval/test
        setting_dict = {'dataset':args.dataset, 'method':args.model, 'sensitive_attr':args.sensitive_attr, 'target_attr':args.target_attr, 'seed':args.seed}
        net.eval()
