...
if __name__ == '__main__':
    ...
    parser.add_argument("--epsilon", type=float, default=0.01, help="epsilon for computing mcdpt")
    parser.add_argument("--K", type=int, default=1, help="K for computing mcdpt")
    ...
    fair_criterion = MaxCDFdp(args.temperature)
    ...
    for step, (X, attr) in enumerate( train_infinite_loader ):
        ...
        net = train_step(model=net, data=X, target=y, sensitive=s, optimizer=optimizer, scheduler=scheduler,  clf_criterion=criterion, fair_criterion=fair_criterion, lam=args.lam, device=device)
    ...
    with torch.no_grad():   # Final eval/test
        ...
        clf_loss, fair_loss, target_hat_list, target_list, sensitive_list = 0, 0, [], [], []
        for X, attr in val_loader:   
            ...
            h, output = net(data)
            clf_loss += criterion(output, target).item()*len(target)
            fair_loss += fair_criterion(output, sensitive).item()*len(target)
            target_hat_list.append(output.cpu().numpy())
            target_list.append(target.cpu().numpy())
            sensitive_list.append(sensitive.cpu().numpy())
        ...
        clf_loss /= len(val_dataset)
        fair_loss /= len(val_dataset)
        ...
        sensitive_list = np.concatenate(sensitive_list, axis=0)
        metric = metric_evaluation(y_gt=target_list, y_pre=target_hat_list, s=sensitive_list, epsilon_list=args.epsilon_list, K=args.K)
        clf_loss /= len(test_dataset)
        fair_loss /= len(test_dataset)
